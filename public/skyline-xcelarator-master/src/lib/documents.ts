...
// --- Filled Air Waybill (the actual executed form, not the guide) -----------
//
// A completed IATA-style Air Waybill — e.g. a Southwest Cargo AWB signed at
// the airport counter and photographed or scanned back for the file. Unlike
// AWB_GUIDE (which only *describes* the form's fields), this is the form
// itself, filled in with a real shipment. It most often arrives as a phone
// photo (no text layer, no positional layout — see ocr.ts's ocrImage), so
// extraction here is deliberately text-only and best-effort: it reads the
// handful of fields whose printed label sits right beside or above its value,
// and leaves the densely tabular routing/charges grid as raw text rather than
// guessing at OCR'd column order.
const AIR_WAYBILL: DocumentDefinition = {
  type: "air-waybill",
  label: "Air Waybill",
  match: ({ text, fileName }) => {
    const flat = flatten(text).toLowerCase();
    const name = fileName.toLowerCase();
    let score = 0;
    // These phrases are printed on the executed form itself, not on the guide
    // that merely lists field names — that keeps the two types from colliding
    // even though they share most of their field labels.
    if (/not negotiable/.test(flat)) score += 0.25;
    if (/issuing carrier'?s agent/.test(flat)) score += 0.2;
    if (/original\s*\d\s*\(for\s+(shipper|consignee|issuing carrier|carrier)\)/.test(flat))
      score += 0.2;
    if (/executed on \(date and time\)/.test(flat) || /signature of issuing carrier/.test(flat))
      score += 0.15;
    if (/total prepaid|total collect/.test(flat)) score += 0.15;
    if (/rate class|chargeable weight|commodity item no/.test(flat)) score += 0.15;
    if (/swacargo|deltacargo|aacargo|unitedcargo|\bcargo\.com\b/.test(flat)) score += 0.1;
    if (/awb|airwaybill|air.?waybill/.test(name)) score += 0.1;
    return Math.min(score, 1);
  },
  extract: ({ text }) => {
    // "Not Negotiable" boxes usually sit right beside the prefix-serial AWB
    // number (e.g. "526-43212761"); fall back to the first such number anywhere.
    // These two run over the whole-document flattened text (capture() always
    // flattens), so neither depends on where line breaks land.
    const awbNumber =
      capture(text, /Not Negotiable[\s\S]{0,120}?(\d{3}[-\s]\d{7,8})/i) ??
      capture(text, /\b(\d{3}-\d{7,8})\b/);

    // Shipper/consignee blocks are large enough, and bounded by clear labels
    // on both sides, to read reliably off the raw (line-broken) text — unlike
    // most of the rest of this form (see below).
    const shipperBlock = text.match(
      /Shipper'?s Name and Address\s*([\s\S]*?)(?:Shipper'?s Account Number|Not Negotiable)/i,
    )?.[1];
    const consigneeBlock = text.match(
      /Consignee'?s Name and Address\s*([\s\S]*?)(?:Consignee'?s Account Number|Issuing Carrier|Not Negotiable)/i,
    )?.[1];

    // The carrier + flight/date box ("WN244 / 08JUN") is a distinctive,
    // self-contained token that can be found anywhere in the flattened text
    // regardless of how OCR breaks the surrounding lines — unlike the rest of
    // this form's routing/charges grid, which packs several header cells onto
    // one shared row and their values onto the row beneath it. Splitting that
    // grid's columns back apart needs the page's positional layout (as the
    // ICAT/CAP alerts do above); a photographed AWB has none, so this
    // extractor stays deliberately narrow rather than guess at column order.
    const flightMatch = flatten(text).match(
      /\b([A-Z]{1,2})(\d{2,4})\s*\/\s*(\d{1,2}[A-Z]{3})\b/i,
    );

    // "Airport of Departure (Addr. of First Carrier) and Requested Routing" is
    // the only header on its row, so the value beneath it is unambiguous.
    const departure = text.match(
      /Airport of Departure[^\r\n]*\r?\n\s*([A-Za-z][A-Za-z .]*)/i,
    )?.[1];

    return [
      { label: "Air Waybill Number", value: awbNumber },
      { label: "Shipper Name and Address", value: composeAddress(shipperBlock) },
      { label: "Consignee Name and Address", value: composeAddress(consigneeBlock) },
      {
        label: "Carrier",
        value: flightMatch ? airlineName(flightMatch[1].toUpperCase()) : null,
      },
      {
        label: "Flight / Date",
        value: flightMatch
          ? `${flightMatch[1].toUpperCase()}${flightMatch[2]} / ${flightMatch[3].toUpperCase()}`
          : null,
      },
      { label: "Airport of Departure", value: cleanValue(departure ?? null) },
    ];
  },
};
...
// --- AIT Worldwide Logistics "Known Shipper" IAC certification --------------
//
// The Known-Shipper counterpart to DHL_IAC above, but issued under AIT
// Worldwide Logistics' own TSA-approved security program rather than DHL Same
// Day's — same purpose (certifying the cargo tendered to an airline), same
// "SENSITIVE SECURITY INFORMATION" cover, different carrier and IAC number
// format (e.g. "GL9312005" rather than DHL's "NE" prefix). Unlike the DHL
// form (a blank template the driver fills by hand), this one arrives mostly
// pre-filled — the known-shipper list, airline, and airbill number are typed
// in already, leaving only the courier's tendering signature blank.
const AIT_IAC_CERTIFICATION: DocumentDefinition = {
  type: "ait-iac-certification",
  label: "AIT Worldwide Logistics Known Shipper Certification",
  match: ({ text, fileName }) => {
    const flat = flatten(text).toLowerCase();
    const name = fileName.toLowerCase();
    let score = 0;
    if (/ait worldwide logistics,? inc is in compliance with its tsa/.test(flat))
      score += 0.5;
    if (/known shipper/.test(flat)) score += 0.2;
    if (/our number assigned by tsa is:?\s*[a-z]{2}\d+/.test(flat)) score += 0.2;
    if (/\biac\b/.test(name) || /known.?shipper/.test(name)) score += 0.1;
    return Math.min(score, 1);
  },
  extract: ({ text }) => {
    const knownShippers = text
      .match(/Known Shippers?:\s*([\s\S]*?)(?:Cargo items which weigh|Signature of IAC)/i)?.[1]
      ?.split("\n")
    ...
    const address = text
      .match(
        /Print Address\/Phone of IAC\/Authorized Representative:\s*([\s\S]*?)(?:WARNING:|$)/i,
      )?.[1]
      ?.split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
    ...
    return [
      ...
      {
        label: "Carrier",
        value: /ait worldwide logistics/i.test(text) ? "AIT Worldwide Logistics, Inc." : null,
      },
      { label: "Date", value: valueAfter(text, "\\bDATE\\b") },
      { label: "Page", value: valueAfter(text, "\\bPAGE\\b") },
      {
        // "Airline I.D.: DL" and "Airbill Number: …" print on the same line,
        // so a plain same-line valueAfter would swallow the second label as
        // part of the first's value — bound it the way DHL_IAC's same-line
        // pairs are handled above.
        label: "Airline I.D.",
        value:
          valueBetweenLabels(text, "Airline I.D.", "Airbill Number") ??
          valueAfter(text, "Airline I\\.D\\."),
      },
      { label: "Airbill Number", value: valueAfter(text, "Airbill Number") },
      { label: "Known Shipper(s)", value: knownShippers ?? null },
      {
        // Likewise shares its line with the "Date:" signature-block label.
        label: "Signature of IAC/Authorized Rep Tendering Cargo",
        value: valueBetweenLabels(
          text,
          "Signature of IAC/Authorized Rep Tendering Cargo",
          "Date",
        ),
      },
      {
        label: "Print Name of IAC/Authorized Rep Tendering Cargo",
        value: valueAfter(text, "Print Name of IAC/Authorized Rep Tendering Cargo"),
      },
      {
        label: "Print Address/Phone of IAC/Authorized Representative",
        value: address ?? null,
      },
    ];
  },
