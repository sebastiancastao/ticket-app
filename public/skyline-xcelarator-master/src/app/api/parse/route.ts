...
import { ocrImage, ocrPdf, TEXT_THRESHOLD } from "@/lib/ocr";
...
// A dispatch ticket or IAC certification forwarded as a phone photo or
// screenshot rather than scanned to PDF — no text layer, no page structure,
// just an image. See parseImageBytes below.
function isImageFile(contentType: string | undefined, name: string) {
...
function unsupportedResult(file: File, name: string): FileResult {
  return {
    ...
    fileName: name,
    fileSize: file.size,
    error: "Not a PDF, EML, or image (JPEG/PNG) file.",
  };
}
...
async function parseImageBytes(input: {
  bytes: Uint8Array;
  name: string;
  fileSize: number;
}): Promise<FileResult> {
  ...
  try {
    // A photographed or screenshotted form has no text layer or page
    // structure at all, so it goes straight through OCR — there's no
    // "extractFormattedText" fallback like the PDF path has, and classifyAndMap
    // gets no positional layout (same as an OCR'd scanned PDF, which also has
    // none). Column-aware templates in documents.ts (ICAT/CAP) return mostly
    // blank fields in that case; text-only templates work unaffected.
    const text = tidy(await ocrImage(bytes));
    return {
      ok: true,
      fileName: name,
      fileSize,
    ...
    console.error(`Image OCR failed for ${name}:`, err);
    return {
      ok: false,
      fileName: name,
      fileSize,
      error: "Failed to read the image (OCR error).",
    };
  }
}
...
async function parseEmlFile(file: File, name: string): Promise<FileResult[]> {
  ...
  try {
    ...
    // A forwarded ticket may arrive as a PDF attachment or, just as often, as
    // a phone photo/screenshot attached directly to the email — route each to
    // the matching parser rather than requiring a PDF specifically.
    const fileAttachments = mail.attachments.filter(
      (attachment) =>
    ...
    if (fileAttachments.length > 0) {
      return Promise.all(
        fileAttachments.map((attachment, i) => {
          const attName = attachmentName(name, attachment, i);
          const bytes = new Uint8Array(attachment.content);
          return isPdfFile(attachment.contentType, attachment.filename ?? "")
            ? parsePdfBytes({ bytes, name: attName, fileSize: attachment.size })
            : parseImageBytes({ bytes, name: attName, fileSize: attachment.size });
        }),
      );
    }
      ...
      return [
        {
          ...
          fileName: name,
          fileSize: file.size,
          error: "No email body text or PDF/image attachments found.",
        },
      ];
    }
...
async function parseFile(input: {
  ...
  if (isImageFile(file.type, name)) {
    const bytes = new Uint8Array(await file.arrayBuffer());
    return [await parseImageBytes({ bytes, name, fileSize: file.size })];
  }
