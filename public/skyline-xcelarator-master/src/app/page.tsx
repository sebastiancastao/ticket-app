...
function downloadName(path: string) {
  ...
  return leaf.replace(/\.(pdf|eml|jpe?g|png)$/i, "") + ".txt";
}
...
function filledFileName(path: string, mapping: DocumentMapping, carrier: Carrier) {
  const leaf = path.split("/").pop() ?? path;
  // The source ticket is usually a PDF, but a dhl-iac cert can also arrive as
  // a phone photo (JPEG/PNG) — strip whichever extension it actually has.
  const stem = leaf.replace(/\.(pdf|jpe?g|png)$/i, "");
...
// Anything /api/parse can turn into text: PDFs, forwarded EML messages
// (parsed server-side for a PDF/image attachment or plain body text), and
// phone photos or screenshots of a paper form (JPEG/PNG) sent straight
// through OCR. The server has long accepted EML; this filter just hadn't
// been letting them reach it.
function isSupportedUpload(file: File, path: string) {
  const lower = path.toLowerCase();
  if (file.type === "application/pdf" || lower.endsWith(".pdf")) return true;
  if (file.type === "message/rfc822" || lower.endsWith(".eml")) return true;
...
export default function Home() {
  ...
  const handleFiles = useCallback(
    async (picked: PickedFile[], chosen: Workflow) => {
      ...
      const supported = picked.filter(({ file, path }) => isSupportedUpload(file, path));
      if (supported.length === 0) {
        setResults([]);
        setError("No PDF, EML, or image files found in the selection.");
        return;
      }
      setError(null);
      setResults([]);
    ...
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          Upload PDFs, EML messages, or photos of a paper ticket — or whole
          folders of them — to extract text and metadata. Files are parsed on
  ...
  return (
    ...
    >
      ...
      <input
        ref={fileInputRef}
        type="file"
        accept="application/pdf,.pdf,.eml,image/jpeg,.jpg,.jpeg,image/png,.png"
        multiple
        className="hidden"
        onChange={(e) => {
