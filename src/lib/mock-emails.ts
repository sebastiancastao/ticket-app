...
//
// Timestamps are computed from a fixed reference instant rather than
// Date.now(): this module is bundled into the client, so it re-runs both
// during SSR and again in the browser at hydration — Date.now() would
// return two different values a few hundred ms apart and produce a
// hydration mismatch once that gap crosses a minute boundary.
const REFERENCE_TIME = new Date("2026-08-19T18:00:00.000Z").getTime();
...
export const MOCK_EMAILS: MockEmail[] = [
  {
    ...
    receivedAt: new Date(REFERENCE_TIME - 1000 * 60 * 35).toISOString(),
  ...
  },
  {
    ...
    receivedAt: new Date(REFERENCE_TIME - 1000 * 60 * 60 * 3).toISOString(),
  ...
  },
  {
    ...
    receivedAt: new Date(REFERENCE_TIME - 1000 * 60 * 60 * 26).toISOString(),
  ...
  },
  {
    ...
    receivedAt: new Date(REFERENCE_TIME - 1000 * 60 * 60 * 48).toISOString(),
