// Ported verbatim from skyline-xcelerator's src/lib/southwest-flight-data.ts.
...
function joinValues(values: Array<string | null>, separator: string): string | null {
...
function parseRoutingLegs(value: string | null): Array<{
  ...
  if (!value) return [];
  return [...value.matchAll(/([A-Z]{3})-([A-Z]{3})\s+([A-Z0-9]{2})\s+\S+/g)].map((match) => ({
    origin: match[1],
    destination: match[2],
    carrierCode: match[3],
  }));
...
export function identifySouthwestFlightData(mapping: DocumentMapping): SouthwestFlightData | null {
  ...
  const flightDates = splitCompoundValue(firstField(mapping, ["Flight Date", "Date Tendered"]));
  ...
  const defaultCarrierName = firstField(mapping, ["Airline Tendered", "Carrier"]);
  ...
  const legs: SouthwestFlightLeg[] = Array.from({ length: legCount }, (_, i) => {
    ...
    return {
      ...
      destination: routingLeg?.destination ?? (i === legCount - 1 ? destinationAirport : null),
      carrierCode: routingLeg?.carrierCode ?? defaultCarrierCode,
      carrierName: airlineName(routingLeg?.carrierCode ?? null) ?? defaultCarrierName,
      flightNumber: flightNumbers[i] ?? null,
      flightDate: flightDates[i] ?? null,
      airWaybillNumber:
        airWaybillNumbers[i] ?? (airWaybillNumbers.length === 1 ? airWaybillNumbers[0] : null),
    };
  });
  ...
  return {
    legs,
    summary: {
      airWaybillNumbers: joinValues(legs.map((leg) => leg.airWaybillNumber), " / "),
      flightDates: joinValues(legs.map((leg) => leg.flightDate), " / "),
      flightNumbers: joinValues(legs.map((leg) => leg.flightNumber), " / "),
      flightNumbersCompact: joinValues(legs.map((leg) => leg.flightNumber), "/"),
    },
  };
}
